        ...
        <div className="relative mx-auto max-w-6xl px-4 pt-10 pb-10 md:pt-16 md:pb-14">
          <div className="grid gap-8 lg:grid-cols-2 lg:items-center">
            ...
            <div className="rounded-none border-4 border-white bg-white shadow-2xl overflow-hidden p-4 relative">
              <div data-lightbox-src={IMG_HERO.src} className="relative aspect-square rounded-none overflow-hidden bg-slate-100 shadow-inner group w-full cursor-pointer"><Image src={IMG_HERO} alt="واير تقطيع خرسانة - Core Pro Egypt" fill priority className="object-cover group-hover:scale-105 transition-transform duration-500 rounded-none" /><div className="absolute inset-0 bg-black/20 opacity-0 group-hover:opacity-100 transition-opacity flex items-center justify-center text-white"><Maximize className="text-white h-10 w-10" /></div></div>
        ...
        <div className="grid gap-10 lg:grid-cols-3">
          <div className="lg:col-span-2 space-y-10">
            ...
            <Section id="comparison" title="متى تختار الواير مقارنة بالبدائل">
              <div className="overflow-x-auto"><table className="w-full text-sm border-collapse"><thead><tr className="bg-slate-900 text-white"><th className="p-3 text-right rounded-tr-2xl">المعيار</th><th className="p-3 text-center">واير ماسي</th><th className="p-3 text-center">منشار سكة</th><th className="p-3 text-center rounded-tl-2xl">كور دريل</th></tr></thead><tbody>{[{ c: "السماكة القصوى", w: "غير محدودة", s: "حتى 70 سم", k: "حتى 60 سم" },{ c: "الاهتزازات", w: "صفر", s: "منخفضة جدا", k: "صفر" },{ c: "الفتحات الدائرية", w: "❌", s: "❌", k: "✅" },{ c: "القطع المستقيم", w: "✅", s: "✅", k: "❌" },{ c: "تكلفة الخدمة", w: "قد تكون أعلى", s: "متوسطة", k: "منخفضة" },].map((row, i) => (<tr key={i} className={i % 2 === 0 ? "bg-white" : "bg-slate-50"}><td className="p-3 font-bold text-slate-800 border-b border-slate-100">{row.c}</td><td className="p-3 text-center text-emerald-700 font-bold border-b border-slate-100">{row.w}</td><td className="p-3 text-center text-sky-700 border-b border-slate-100">{row.s}</td><td className="p-3 text-center text-slate-600 border-b border-slate-100">{row.k}</td></tr>))}</tbody></table></div>
            </Section>
