  ...
  return (
    <section id={id} className="rounded-3xl border border-black/10 bg-white/70 backdrop-blur-xl p-6 md:p-10 shadow-[0_12px_50px_rgba(0,0,0,0.08)] mb-10">
      <div className="mb-6 flex items-start gap-3">
        ...
        <h2 className="text-2xl md:text-3xl font-black tracking-tight text-slate-900 border-r-4 border-emerald-600 pr-3">
          {title}
        </h2>
      </div>
  ...
  return (
    <div className="bg-slate-50" dir="rtl">
      ...
      <div className="mx-auto max-w-5xl px-4 pb-24">
        ...
        {/* Core Pricing */}
        <Section id="core-prices" title="أسعار فتحة الكور وتخريم الخرسانة" icon={<Sparkles className="h-5 w-5 text-sky-600" />}>
          <p className="mb-4">
            تختلف <strong>أسعار الفتحة الكور</strong> بناءً على عوامل أساسية: القطر المطلوب، وسُمك الحائط، ونوعه (خرسانة مسلحة أم مباني طوب).
          </p>
          <div className="overflow-x-auto rounded-2xl border border-black/10 shadow-sm mb-6">
            <table className="w-full text-sm border-collapse bg-white">
              <thead>
                <tr className="bg-slate-900 text-white">
                  <th className="p-4 text-right">نوع الفتحة / القطر</th>
                  <th className="p-4 text-right">أبرز الاستخدامات</th>
                  <th className="p-4 text-right">ملاحظات التسعير</th>
                </tr>
              </thead>
              <tbody>
                <tr className="border-b border-slate-100 p-4">
                  <td className="p-4 font-bold text-slate-800">1 بوصة إلى 2 بوصة</td>
                  <td className="p-4 text-slate-600">لشنيشة الغاز، تأسيس السباكة الرفيعة.</td>
                  <td className="p-4 text-slate-600">تسعير اقتصادي للكميات للمقاولين والمجمعات.</td>
                </tr>
                <tr className="border-b border-slate-100 p-4 bg-slate-50">
                  <td className="p-4 font-bold text-slate-800">3 بوصة إلى 4 بوصة</td>
                  <td className="p-4 text-slate-600">تهوية بسيطة، تصريف تكييف، مواسير صرف صحي.</td>
                  <td className="p-4 text-slate-600">سعر متوسط يختلف لـ (جدار طوب vs تسليح كثيف).</td>
                </tr>
                <tr className="p-4">
                  <td className="p-4 font-bold text-slate-800">5 بوصة إلى 8 بوصة</td>
                  <td className="p-4 text-slate-600"><strong>تخريم فتحة الشفاط</strong>، مداخن السخانات، والمصانع.</td>
                  <td className="p-4 text-slate-600">متوفر عرض (تخريم الكور + تركيب الشفاط) لتوفر التكلفة.</td>
                </tr>
              </tbody>
            </table>
          </div>
        ...
        {/* Saw Pricing */}
        <Section id="saw-prices" title="أسعار قص الخرسانة بالمنشار وفتح الأبواب" icon={<Building2 className="h-5 w-5 text-amber-600" />}>
          <p className="mb-4">
          ...
          <ul className="space-y-3 mb-6 list-none p-0">
            <li className="flex gap-2 items-start"><CheckCircle2 className="h-5 w-5 text-emerald-600 shrink-0" /><span><strong>قياس المتر الطولي (Linear Meter):</strong> نقيس إجمالي أطوال خطوط القطع المطلوبة.</span></li>
            <li className="flex gap-2 items-start"><CheckCircle2 className="h-5 w-5 text-emerald-600 shrink-0" /><span><strong>سُمك القطاع (Thickness):</strong> سعر المنشار لجدار 15 سم أقل من سعر جدار خرساني 25 سم أو سقف سميك.</span></li>
            <li className="flex gap-2 items-start"><CheckCircle2 className="h-5 w-5 text-emerald-600 shrink-0" /><span><strong>نوع الماكينة (Equipment):</strong> قص صاروخ للأماكن الصغيرة، والقص بالمنشار الحائطي الكبير، أوقص الأرضيات (Slab Saw).</span></li>
          </ul>
        ...
        <Section id="hood-packages" title="باقات تركيب شفاط المطبخ والحمام" icon={<ShieldCheck className="h-5 w-5 text-emerald-600" />}>
            ...
            {faq.map((item, i) => (
              <details key={i} className="group rounded-3xl border border-black/10 bg-white p-6 shadow-sm hover:border-emerald-300 transition">
                <summary className="cursor-pointer list-none font-bold text-slate-900 flex items-center justify-between text-lg">
                  <span>{item.q}</span>
                  <span className="text-emerald-500 group-open:rotate-180 transition p-1 bg-emerald-50 rounded-full">⌄</span>
                </summary>
