  ...
  return (
    <section id={id} className={`rounded-3xl border border-black/10 bg-white/70 backdrop-blur-xl p-6 md:p-10 shadow-[0_12px_50px_rgba(0,0,0,0.08)] ${level === 3 ? 'mx-4 md:mx-8' : ''}`}>
      <div className="mb-6 flex items-start gap-3">
        {icon ? (<div className="mt-1 inline-flex h-11 w-11 items-center justify-center rounded-2xl bg-black/5">{icon}</div>) : null}
        <div>
          <HeadingTag className={`font-black tracking-tight text-slate-900 border-r-4 border-orange-500 pr-3 ${level === 2 ? 'text-2xl md:text-3xl' : 'text-xl md:text-2xl'}`}>{title}</HeadingTag>
          ...
          <div className="grid gap-8 lg:grid-cols-2 lg:items-center">
            <div>
              <div className="inline-flex items-center gap-2 rounded-full border border-orange-200 bg-orange-50 px-4 py-2 text-sm text-orange-800 shadow-sm">
                <Fan className="h-4 w-4 text-orange-500" />
            ...
            <div className="rounded-none border-4 border-white bg-white shadow-2xl overflow-hidden p-4 relative">
              <div data-lightbox-src={IMG_HERO.src} className="relative aspect-square rounded-none overflow-hidden bg-slate-100 shadow-inner group w-full cursor-pointer">
                <Image src={IMG_HERO} alt="تركيب شفاط مطبخ مدخنة - Core Pro Egypt" fill priority className="object-cover group-hover:scale-105 transition-transform duration-500 rounded-none" />
      ...
      <div className="mx-auto max-w-6xl px-4 pb-24">
        <div className="grid gap-10 lg:grid-cols-3">
          <div className="lg:col-span-2 space-y-10">
            ...
            <Section id="why-us" title="لماذا تختار فنيينا لتركيب شفاط المطبخ؟" icon={<Star className="h-5 w-5 text-orange-500" />}>
              <p>
              ...
              <div className="mt-8 grid grid-cols-1 md:grid-cols-3 gap-4">
                {[
                  { icon: <Zap className="h-5 w-5 text-orange-500" />, title: "إنجاز في نفس اليوم", desc: "زيارة واحدة تكفي من التخريم حتى التسليم" },
                  { icon: <ShieldCheck className="h-5 w-5 text-orange-500" />, title: "ضمان التثبيت", desc: "نضمن عزل تام ضد رجوع الروائح والهواء" },
                  { icon: <CheckCircle2 className="h-5 w-5 text-orange-500" />, title: "بدون تكسير", desc: "تخريم بالكور دون أي ضرر للدهانات أو السيراميك" },
                ].map((f, i) => (
            ...
            <Section id="brands" title="الماركات التي نركبها ونصلحها" icon={<BadgeCheck className="h-5 w-5 text-orange-500" />}>
              <p className="mb-6">
              ...
              <div className="grid grid-cols-2 md:grid-cols-3 gap-3">
                {[
                  ...
                  <div key={b} className="flex items-center gap-2 rounded-2xl border border-black/10 bg-white p-3 text-sm font-bold text-slate-700">
                    <CheckCircle2 className="h-4 w-4 text-orange-500 shrink-0" />{b}
                  </div>
                ))}
              </div>
            ...
            <Section id="faq" title="الأسئلة الشائعة حول تركيب الشفاطات" icon={<HelpCircle className="h-5 w-5 text-orange-500" />}>
                ...
                {faq.map((item, i) => (
                  <details key={i} className="group rounded-3xl border border-black/10 bg-white p-6 shadow-sm hover:border-orange-300 transition">
                    <summary className="cursor-pointer list-none font-bold text-slate-900 flex items-center justify-between text-lg">
                      <span>{item.q}</span>
                      <span className="text-orange-500 group-open:rotate-180 transition p-1 bg-orange-50 rounded-full">⌄</span>
                    </summary>
          ...
          <aside className="space-y-6">
            <div className="p-8 rounded-[40px] bg-white border border-black/10 shadow-lg">
              ...
              <div className="space-y-4">
                <StatPill icon={<CheckCircle2 className="h-4 w-4 text-orange-500" />} text="تخريم بدون غبار" />
                <StatPill icon={<Zap className="h-4 w-4 text-orange-500" />} text="إنجاز في نفس اليوم" />
                <StatPill icon={<MapPin className="h-4 w-4 text-orange-500" />} text="نخدم جميع المناطق" />
                <StatPill icon={<ShieldCheck className="h-4 w-4 text-orange-500" />} text="ضمان التثبيت والعزل" />
              </div>
            </div>
