            ...
            <nav className="hidden lg:flex items-center gap-1 relative">
              ...
              {megaOpen && (
                <div
                  onMouseEnter={() => setMegaOpen(true)}
                  onMouseLeave={() => setMegaOpen(false)}
                  className="absolute top-14 right-0 w-72 bg-white border rounded-2xl shadow-xl p-4 space-y-2"
                >
                  {extraPages.map((item) => (
                    <Link
                      key={item.href}
        ...
        <div className="lg:hidden fixed inset-0 z-[999] bg-white">
          ...
          <div className="flex items-center justify-between border-b p-4">
            <div className="flex items-center gap-3">
              <div className="grid h-9 w-9 place-items-center rounded-2xl bg-slate-900 text-white font-extrabold">
                CP
              </div>
            ...
            <div className="pt-4 border-t mt-4 space-y-2">
              {extraPages.map((item) => (
                <Link
                  ...
                  href={item.href}
                  onClick={() => setOpen(false)}
                  className="block px-4 py-3 rounded-2xl border border-black/10 font-bold text-slate-700"
                >
                  {item.label}
                </Link>
              ))}
